...
describe("Basic test", () => {
  it("should works with basic attributes", () => {
    ...
    const query = sufixes.reduce(
      ...
      { _limit: 10, _start: 50, _sort: "name", _order: "-1", _select: "-name" }
    );

    const aggregation = processQuery(model.schema, query);

    sufixes.forEach((sufix) => {
      if (/^regexi?$/.test(sufix)) {
        const options = sufix === "regexi" ? { $options: "i" } : {};
        expect(aggregation).to.deep.include({ $match: { name: { $regex: sufix, ...options } } });
      } else if (/^in$/.test(sufix)) {
        expect(aggregation).to.deep.include({ $match: { name: { $in: [sufix] } } });
      } else if (/^nin$/.test(sufix)) {
        expect(aggregation).to.deep.include({ $match: { name: { $not: { $in: [sufix] } } } });
      } else {
        expect(aggregation).to.deep.include({ $match: { name: { [`$${sufix}`]: sufix } } });
      }
    });
